...
public class ChooseController {
    ...
    public String choose(@Valid @RequestBody ChooseRequest chooseRequest) {
        ...
        for(int i = 0;i<ChosenCouIdList.size();i++){
            ...
            if( (ChosenIndex == theIndex) && (ChosenWeek == theWeek) ){
                List<Integer> choseStuIdList =  selectionsRepository.getChosenStuIdListByCouid(ChosenCouId);
                for(int j = 0;j<choseStuIdList.size();j++){
                    Integer ChosenStuId = choseStuIdList.get(j);
                    if(ChosenStuId == studentId)
                        return "时间冲突";
                }
    ...
    private Integer cancelChoose = 0;
    @PostMapping("/cancelChose")
    public String cancelChose(@Valid @RequestBody IdentifyRoleRequest identifyRoleRequest){
        String username = identifyRoleRequest.getUsername();
        Integer role = userRepository.getRole(username);
    ...
    @PostMapping("/openChose")
    public String openChose(@Valid @RequestBody IdentifyRoleRequest identifyRoleRequest){
        String username = identifyRoleRequest.getUsername();
        Integer role = userRepository.getRole(username);
    ...
    @PostMapping("/weatherCancelChose")
