...
public interface SelectionsRepository extends JpaRepository<Selections, Integer> {
    ...
    @Query(value = "SELECT student_id FROM selections WHERE course_id = ?1", nativeQuery = true )
    List<Integer> getChosenStuIdListByCouid(Integer courseId);
    ...
    @Query(value = "SELECT student_id from selections where course_id =?1", nativeQuery = true )
    Integer getStuIdByCourseId(Integer CourseId);
